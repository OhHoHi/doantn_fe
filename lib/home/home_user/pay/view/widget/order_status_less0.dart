...
class OrderStatusLess0 extends StatelessWidget {
  OrderStatusLess0({Key? key, required this.isAdmin , required this.initialTabIndex})
      : super(key: key);
  bool isAdmin ;
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      ...
      child: BodyOrderStatusLess0(isAdmin: isAdmin, initialTabIndex: initialTabIndex),
    );
  }
}

class BodyOrderStatusLess0 extends StatefulWidget {
  BodyOrderStatusLess0({Key? key , required this.isAdmin , required this.initialTabIndex}) : super(key: key);
  bool isAdmin ;
  final int initialTabIndex;
  @override
  State<BodyOrderStatusLess0> createState() => _OrderStatusLess0State();
}

class _OrderStatusLess0State extends State<BodyOrderStatusLess0> {
  ...
  void initState() {
    ...
    if(widget.isAdmin == true){
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        paymentProvider.getListOrderAllStatusLess0();
      });
    }
    else{
      WidgetsBinding.instance.addPostFrameCallback((_) {
        //  paymentProvider.resetPage();
        paymentProvider.getListOrderStatusLess0WithUser(user.user.id);
        ...
        return
          ...
          SingleChildScrollView(
            child: ListView.builder(
              ...
              shrinkWrap: true,
              itemCount: paymentProvider.listOrderStatusLess0.length,
              itemBuilder: (context, index) {
                return buildData(paymentProvider,paymentProvider.listOrderStatusLess0[index]);
              },
            ),
            //  ),
          );
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: Row(
                          children: [
                            Text("Đã hủy", style: AppStyles.nuntio1_14_red),
                            const Spacer(),
                            // if (widget.isAdmin == true)
                            //   ElevatedButton(
                            //     onPressed: () {
